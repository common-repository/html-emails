...
// TODO: Need to plug in to pre_site_option_registrationnotification and return 'no'. But, need to watch for other places it's called.
-- newblog_notify_siteadmin (no pluggable filter)
-- newuser_notify_siteadmin (no pluggable filter)
-- wpmu_welcome_notification
-- wpmu_welcome_user_notification
- Better Plain Text version
- Add translations
- For MS need to change email footer and make it site specific

*/

define( 'HTMLIZE_DEBUG', WP_DEBUG );
...
function htmlize() {
	...
	if( is_multisite() ) {
		add_filter( 'wpmu_signup_blog_notification', 'htmlize_wpmu_signup_blog_notification', 10, 7 );
		add_filter( 'wpmu_signup_user_notification', 'htmlize_wpmu_signup_user_notification', 10, 4 );
		
		add_filter( 'wpmu_welcome_notification', 'htmlize_wpmu_welcome_notification', 10, 5 );
		add_filter( 'wpmu_welcome_user_notification', 'htmlize_wpmu_welcome_user_notification', 10, 3 );
	}
	
	if( HTMLIZE_DEBUG_SEND ) htmlize_debug();
}
...
function htmlize_message( $data ) {
	...
	if( ! isset( $parent_template ) || ! $parent_template ) $parent_template = HTMLIZE_BASE_TEMPLATE;
