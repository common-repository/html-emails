...
function htmlize_get_admin_email() {
	return get_site_option( 'admin_email' ) == '' ? 'support@' . $_SERVER['SERVER_NAME'] : get_site_option( 'admin_email' );
	return $admin_email;
}

function htmlize_get_from_name() {
	return get_site_option( 'site_name' ) == '' ? 'WordPress' : esc_html( get_site_option( 'site_name' ) );
}

function htmlize_get_message_headers( $headers = '' ) {
	return sprintf( 'Content-Type: text/html; charset="%s"\n %s', get_option( 'blog_charset' ), $headers );
}

function htmlize_get_from_header( $from_name = '', $from_email = '' ) {
	if( ! $from_name )
		$from_name = htmlize_get_from_name();
	if( ! $from_email )
		$from_email = htmlize_get_admin_email();
		
	return sprintf( 'From: "%1$s" <%2$s>\n', $from_name, $from_email );
}
...
function htmlize_get_email_logo( ) {
	...
	return apply_filters( 'htmlize_email_logo', $email_logo );
}
...
function htmlize_debug() {
	if( defined( 'HTMLIZE_DEBUG_SEND' ) && HTMLIZE_DEBUG_SEND ) {
		wp_notify_postauthor(1, 'comment');
		wp_notify_postauthor(1, 'trackback');
		wp_notify_moderator(1);
		wp_new_user_notification(1, 'abcd');
		wp_password_change_notification(new WP_User(1));
	
		wpmu_signup_user_notification( 'admin', 'batmoo@gmail.com', '', '' );
		wpmu_signup_blog_notification( 'localhost', '/my-blog', 'My Amazing Blog', 'joesmith', 'batmoo@gmail.com', '', '' );
		wpmu_welcome_notification( 1, 1, 'password123', 'My Super Blog', '' );
		wpmu_welcome_user_notification( 1, 'password123', '' );
	}
}
?>